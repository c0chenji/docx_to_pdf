...
class Convert:
  def __init__(self, filename, input_list, output):
    self.filename = filename
    self.input_list = input_list
    self.output = output

  def process(self):
    paragraphs = Docx(self.filename).read()
...
if __name__ == "__main__":
    ...
    target = Convert("test1.docx",["John",23],"output1_updated.pdf")
    target.process()
    target = Convert("test2.docx",["Jack",23,1231231,"balba"],"output2_updated.pdf")
    target.process()
